...
const TEAM_ROSTERS = {
  ...
  "Team 2": ["Saikat", "Dhiraj S", "SKP", "Dhiraj W", "Viswas", "Pramila"],
  "Team 3": ["Shankar", "Ananth", "Santhosh", "Srividya", "Aditya", "Dhwani"],
...
const SCHEDULE_DATA = {
  ...
  "Feb 8": [    
    ...
    { time: "08:00 PM", type: "Award Ceremony", pool: "All", t1: "", t2: "" }
  ]
};
...
const draftingData = useMemo(() => {
  ...
  return processed.map((p, index) => {
    ...
    else if (index < 21) tier = "B"; // Next 14 are Core Players
    return { ...p, tier };
  });
}, [history]);
      ...
      border: "none"}}>{tab.toUpperCase()}</button>
            ...
            {infoTab === "banner" && isAdmin && (
               <div style={{ padding: "20px", background: theme.card, borderRadius: "15px", border: `1px solid ${theme.accent}` }}>
                 ...
                 <textarea rows="3" value={bannerText} onChange={(e) => updateBanner(e.target.value)} style={{ width: "100%", background: "#000", color: "#FFF", border: "1px solid #333", padding: "12px", borderRadius: "8px", fontSize: "16px", outline: "none", resize: "none" }} />
               </div>
            )}
  ...
  <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
    ...
    <div style={{ padding: "20px", background: theme.card, borderRadius: "15px", border: "1px solid #333" }}>
      ...
      <textarea 
        ...
        style={{ width: "100%", background: "#000", color: "#FFF", border: "1px solid #333", padding: "12px", borderRadius: "8px", fontSize: "16px", outline: "none", resize: "none" }} 
      />
      <button 
        onClick={() => {
...
{infoTab === "rules" && (
  <div style={{ padding: "20px", background: theme.card, borderRadius: "15px", border: "1px solid #333" }}>
    <ul style={{ color: "#EEE", lineHeight: "2.2", margin: 0, paddingLeft: "20px", fontSize: "14px" }}>
      ...
      <li>7 point Tie-breaker( lead by 2) in case of 6-6 </li>
      <li>2 Matches per player is a must in league round </li>
      <li>2nd Duece is a Decider/Golden point. Receiver chooses the side on Decider</li>
...
{/* 1. TEAMS TAB: Grouped by Pool with restored GMS and SETS */}
{infoTab === "team_std" && (
              ...
              <tbody>
                {poolTeams.map((t, i) => (
                  <tr key={t.name} style={{ borderBottom: i === poolTeams.length - 1 ? "none" : "1px solid #222" }}>
                    ...
                    <td style={{ textAlign: "center", color: "#888" }}>{t.games}</td>
